...
function App() {
  ...
  useEffect(() => {
    const verifyAuth = async () => {
      ...
      if (user && location.pathname !== '/login' && location.pathname !== '/register' && !location.pathname.startsWith('/features') && location.pathname !== '/contact') {
        // Only block UI on initial load, not during navigation
        ...
        // For public routes, mark initial load as complete
        setInitialLoadComplete(true);
      }
    };

    verifyAuth();
  }, [location.pathname, user, checkAuthStatus, navigate, initialLoadComplete]);
  ...
  if (!initialized || (isVerifyingAuth && !initialLoadComplete)) {
    return (
