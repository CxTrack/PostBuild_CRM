...
const MarkAsDropdown: React.FC<MarkAsDropdownProps> = ({ 
  currentStatus, 
  onStatusChange,
  disabled = false
}) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-dark-800 ring-1 ring-black ring-opacity-5 z-10 border border-dark-700">
            ...
            {statuses.map((status) => (
              <button
                ...
                className={`block w-full text-left px-4 py-2 text-sm hover:bg-dark-700 ${
                  status === currentStatus ? 'bg-dark-700' : ''
                }`}
                role="menuitem"
              >
