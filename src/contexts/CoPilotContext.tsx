...
export interface Message {
  ...
  /** True when this user message was auto-generated by the system (quarterback, meeting prep, etc.) -- renders as collapsible thinking bubble instead of purple user bubble */
...
interface CoPilotContextType {
  ...
  sendMessage: (content: string, options?: { isSystemGenerated?: boolean }) => Promise<void>;
  clearMessages: () => void;
...
export const CoPilotProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const SYSTEM_PROMPT_PREFIXES = ['[QUARTERBACK_MODE]', '[MEETING_PREP_MODE]', '[PERSONALIZATION_COMPLETE]'];

  const sendMessage = useCallback(async (content: string, options?: { isSystemGenerated?: boolean }) => {
    // Explicit flag takes priority; prefix detection as safety net for any callers that forget
    const isSystemGenerated = options?.isSystemGenerated ??
      SYSTEM_PROMPT_PREFIXES.some(p => content.startsWith(p));
    ...
    if (hasOtherText) {
      ...
      try {
        ...
        if (nextQ) {
          ...
          setTimeout(() => { sendMessage(summaryMsg, { isSystemGenerated: true }); }, 700);
        }
      } finally {
