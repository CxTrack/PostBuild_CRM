/**
 * mark-provider-email-read
 * Marks emails as read in the user's connected email provider (Gmail / Outlook).
...
// ── Provider-specific mark-read operations ───────────────────────────────────

async function markReadGmail(token: string, messageId: string): Promise<{ marked: boolean; error?: string }> {
  ...
  if (!res.ok) {
    ...
    return { marked: false, error: err.error?.message || `Gmail mark-read failed (${res.status})` }
  }
  return { marked: true }
}

async function markReadOutlook(token: string, messageId: string): Promise<{ marked: boolean; error?: string }> {
  const res = await fetch(
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({}))
    return { marked: false, error: err.error?.message || `Outlook mark-read failed (${res.status})` }
  }
  return { marked: true }
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const email of emails) {
      ...
      if (!conn) {
        results.push({ message_id, provider, marked: false, error: `No ${provider} connection found` })
        continue
      }
      ...
      if (!(provider in tokenCache)) {
        ...
        if (!resolved.token) {
          results.push({ message_id, provider, marked: false, error: resolved.error })
          continue
        }
      }
      ...
      if (!token) {
        results.push({ message_id, provider, marked: false, error: 'Token unavailable' })
        continue
      }

      try {
        const result = provider === 'google'
          ? await markReadGmail(token, message_id)
          : await markReadOutlook(token, message_id)
        results.push({ message_id, provider, ...result })
      } catch (err) {
        results.push({ message_id, provider, marked: false, error: String(err) })
      }
    }

    const marked = results.filter(r => r.marked).length
    const failed = results.filter(r => !r.marked).length
