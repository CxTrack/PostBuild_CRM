...
BEGIN
  ...
  -- Add priority column with validation
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'customers' AND column_name = 'priority'
  ) THEN
    ALTER TABLE customers ADD COLUMN priority text DEFAULT 'Low'::text;
  END IF;
END $$;
...
ALTER TABLE customers DROP CONSTRAINT IF EXISTS customers_priority_check;
